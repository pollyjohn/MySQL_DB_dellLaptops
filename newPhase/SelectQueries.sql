...
SELECT memory_size, COUNT(*) FROM memory GROUP BY memory_size HAVING COUNT(*) > 1;
...
FROM
    products p
JOIN
    product_has_graphic_card pg ON p.id = pg.products_id
JOIN
    graphic_cards g ON pg.graphic_card_id = g.id
JOIN
    product_has_processor pp ON p.id = pp.products_id
JOIN
    processor pr ON pp.processor_id = pr.id
JOIN
    product_has_memory pm ON p.id = pm.products_id
JOIN
    memory m ON pm.memory_id = m.id
JOIN
    product_has_storage ps ON p.id = ps.products_id
JOIN
    storage s ON ps.storage_id = s.id
JOIN
    product_has_screen_size pss ON p.id = pss.product_id
JOIN
    screen_size ss ON pss.screen_size_id = ss.id
JOIN
    product_has_resolution prs ON p.id = prs.products_id
JOIN
    Resolution r ON prs.Resolution_id = r.id
JOIN
    product_has_color pc ON p.id = pc.products_id
JOIN
...
FROM
    products p
JOIN
    product_has_processor php ON p.id = php.products_id
JOIN
    processor pr ON php.processor_id = pr.id
JOIN
    product_has_graphic_card phgc ON p.id = phgc.products_id
JOIN
    graphic_cards gc ON phgc.graphic_card_id = gc.id
JOIN
    product_has_memory phm ON p.id = phm.products_id
JOIN
    memory m ON phm.memory_id = m.id
JOIN
    product_has_screen_size phss ON p.id = phss.product_id
JOIN
    screen_size s ON phss.screen_size_id = s.id
JOIN
    product_has_resolution phr ON p.id = phr.products_id
JOIN
    Resolution re ON phr.Resolution_id = re.id
JOIN
    product_has_storage phst ON p.id = phst.products_id
JOIN
    storage st ON phst.storage_id = st.id
JOIN
    product_has_color phc ON p.id = phc.products_id
JOIN
    Color c ON phc.Color_id = c.id;
    
SELECT * FROM products;
SELECT * FROM product_has_processor;